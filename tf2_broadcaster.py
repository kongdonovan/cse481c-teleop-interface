...
class FixedFrameBroadcaster():
    ...
    def __init__(self):
        ...
        # Repeat broadcast transform for fk_link_lift
        self.arm = TransformStamped()
        self.arm.header.stamp = rospy.Time.now()
        self.arm.header.frame_id = 'link_arm_l0'
        self.arm.child_frame_id = 'fk_link_arm_l0'
        self.arm.transform.translation.x = 0.0
        self.arm.transform.translation.y = 1.0
        self.arm.transform.translation.z = 0.0
        q = tf.transformations.quaternion_from_euler(1.5707, 0, -1.5707)
        self.arm.transform.rotation.x = q[0]
        self.arm.transform.rotation.y = q[1]
        self.arm.transform.rotation.z = q[2]
        self.arm.transform.rotation.w = q[3]

        # Publish transforms in a list
        self.br.sendTransform([self.mast, self.lift, self.wrist, self.arm])
